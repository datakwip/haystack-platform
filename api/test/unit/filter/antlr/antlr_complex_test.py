...
user_id = 2
db_schema = "core_dev"
class AntlrComplexTest(unittest.TestCase):
    ...
    def test_name_path(self):
        ...
        expected_result = """WITH RECURSIVE hier_query1
          ...
          (
          select child_id, parent_id
          from core_dev.tag_hierarchy th, core_dev.tag_def td4
          where th.child_id  = td4.id
          and td4.name = 'dis'
          UNION ALL
          select th2.child_id, th2.parent_id
          from core_dev.tag_hierarchy th2
          INNER JOIN hier_query4 c ON c.parent_id = th2.child_id
          )
          select root.id entity_id from (select a.id from (select e.id, et.tag_id
           ...
           || string_agg(td4.name, ',') 
           || ',' as parent_id  
           from hier_query4, core_dev.tag_def td4
           where td4.id = hier_query4.parent_id) hq4
            where  e.id = et.entity_id AND (  e.id in ( (select entity_id
            ...
            select 1
            from  core_dev.entity_tag et4, core_dev.tag_def td4
            where et4.entity_id = e.id
                  and et4.tag_id = td4.id
                  and td4.name = 'dis'
                  AND CASE
        WHEN hq4.parent_id like '%,str,%'
            THEN   et4.value_s = 'Zone B'
        ...
        result = get_sql("equipRef->siteRef->yearBuilt==1985 and dis==\"Zone B\"", org_id, user_id, db_schema)
        self.assertEqual(expected_result, result)

    def test_name_path2(self):
        database = Database("postgresql://postgres:password@host.docker.internal/master")
        database.init_database()
        db = database.get_local_session()
        expected_result = """WITH RECURSIVE hier_query1
        ...
        (select entity_id
            from core_dev.entity_tag et2, core_dev.tag_def td2
            where et2.tag_id = td2.id and td2.name = 'equipRef')
        ...
        result = get_sql("dis==\"Zone B\" and equipRef", org_id, user_id, db_schema)
        print (result)
        self.assertEqual(expected_result, result)
